...
#     input_shape = (320,320) # multiple of 32, hw
    ...
    print(f'is tiny ? {is_tiny_version}')
    if is_tiny_version:
    ...
    if True:
        ...
        #add for last checkpoint referenced https://github.com/qqwweee/keras-yolo3/issues/652
        checkpoint_epoch = 0 
        if is_history_exist(log_dir):
            checkpoint_path, checkpoint_epoch = get_minimum_loss_weight(log_dir)
            ...
            print('checkpoint files are not exsisting.')
            model.fit_generator(data_generator_wrapper(lines[:num_train], batch_size, input_shape, anchors, num_classes),
                steps_per_epoch=max(1, num_train//batch_size),
                validation_data=data_generator_wrapper(lines[num_train:], batch_size, input_shape, anchors, num_classes),
                validation_steps=max(1, num_val//batch_size),
